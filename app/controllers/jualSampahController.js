...
const uploadImageUtils = require("../utils/uploadImage");

class JualSampahController {
  async createJualSampah(req, res) {
    const image = req.file.path;
    try {
      const imageUrl = await uploadImageUtils.uploadJualmage(image);
      const data = {
        foto_sampah: imageUrl,
        ...req.body,
      };
      const jualSampah = await jualSampahService.createJualSampah(data);
      res.status(201).json({
